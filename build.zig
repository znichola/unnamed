...
pub fn build(b: *std.Build) !void {
    ...
    if (target.query.os_tag == .emscripten) {
        ...
        const run_option = b.step("run", "Run OOEP");
        run_option.dependOn(&run_step.step);
        return;
    }

    const exe = b.addExecutable(.{ .name = "OOEP", .root_source_file = b.path("src/main.zig"), .optimize = optimize, .target = target });
    ...
    const run_step = b.step("run", "Run OOEP");
